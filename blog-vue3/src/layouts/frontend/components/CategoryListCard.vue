<template>
    <div v-if="categories && categories.length > 0" 
        ...
        <!-- Category header with title and view more button -->
        <div class="flex mb-3">
        ...
        <div class="text-sm flex flex-wrap gap-3 font-medium text-gray-600 rounded-lg dark:border-gray-600 dark:text-white">
            <a @click="goCategoryArticleListPage(category.id, category.name)" v-for="(category, index) in categories"
                ...
                {{ category.name }}
                <span
                    class="inline-flex items-center justify-center w-4 h-4 ms-2 text-xs font-semibold text-emerald-800 bg-emerald-200 rounded-full dark:bg-emerald-800 dark:text-emerald-200">
