<template>
    <div v-if="tags && tags.length > 0" 
        ...
        <!-- Tag header with title and view more button -->
        <div class="flex mb-3">
            ...
            <a @click="router.push('/tag/list')" 
                class="bg-gray-100 flex items-center px-2.5 py-1 text-xs font-medium text-center 
                ...
                dark:focus:ring-gray-700 cursor-pointer transition-colors duration-200">
        ...
        <div class="text-sm flex flex-wrap gap-3 font-medium text-gray-600 rounded-lg dark:border-gray-600 dark:text-white">
            <a @click="goTagArticleListPage(tag.id, tag.name)" v-for="(tag, index) in tags" :key="index"
                ...
                dark:border-gray-700 dark:hover:text-white transition-colors duration-200">
                {{ tag.name }}
                <span
                    class="inline-flex items-center justify-center w-4 h-4 ms-2 text-xs font-semibold text-sky-800 bg-sky-200 rounded-full dark:bg-sky-800 dark:text-sky-200">
                    {{ tag.articlesTotal || 0 }}
                </span>
            </a>
        </div>
