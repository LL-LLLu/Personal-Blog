<template>
  <div class="min-h-screen flex flex-col">
    ...
    <!-- Main content area -->
    <main class="container max-w-screen-xl mx-auto p-4 flex-grow">
    ...
    <div class="grid grid-cols-4 gap-7">
      ...
      <div class="col-span-4 md:col-span-3 mb-3">
        ...
        <div
          ...
          :key="index"
          class="p-6 mb-6 border border-gray-200 rounded-xl bg-white dark:bg-gray-800 dark:border-gray-700 shadow-sm hover:shadow-md transition-all duration-200"
        >
          <div class="flex items-center mb-4 pb-3 border-b border-gray-100 dark:border-gray-700">
            <div class="w-3 h-3 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full mr-3 animate-pulse" />
            <time class="text-xl font-bold text-gray-900 dark:text-white tracking-wide">{{ archive.month }}</time>
            <div class="ml-auto text-sm text-gray-500 dark:text-gray-400 font-medium">
              {{ archive.articles.length }} articles
            </div>
          </div>
          <ol class="space-y-2">
            <li
              v-for="(article, index2) in archive.articles"
              :key="index2"
            >
              <a
                class="items-center block p-4 sm:flex hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg cursor-pointer group transition-all duration-200 hover:shadow-sm"
                @click="goArticleDetailPage(article.id)"
              >
                <img
                  class="w-28 h-16 mb-3 mr-4 rounded-lg sm:mb-0 object-cover group-hover:scale-105 transition-transform duration-200 shadow-sm"
                  :src="article.cover"
                >
                <div class="flex-1">
                  ...
                  <div class="flex items-center justify-between">
                    <span class="inline-flex items-center text-sm font-medium text-gray-500 dark:text-gray-400">
                      <svg
                        class="inline w-4 h-4 mr-2 text-gray-400 dark:text-gray-500"
                        aria-hidden="true"
