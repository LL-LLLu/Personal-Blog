    ...
    <div>
        ...
        <el-card shadow="never" class="mb-5">
            ...
            <div class="flex items-center">
                ...
                <div class="ml-3 w-52 mr-5"><el-input v-model="searchRouterUrl" placeholder="Enter (fuzzy search)" clearable />
                </div>
        ...
        <el-card shadow="never">
            ...
            <el-table :data="tableData" border stripe v-loading="tableLoading" table-layout="auto">
                ...
                <el-table-column prop="routerUrl" label="Router">
                    <template #default="scope">
                        <el-link type="primary" :href="'#' + scope.row.routerUrl" target="_blank">{{ scope.row.routerUrl
                            }}</el-link>
                    </template>
                </el-table-column>
                <el-table-column prop="avatar" label="Avatar" width="60">
                    <template #default="scope">
                ...
                <el-table-column prop="createTime" label="Published Time" width="200" />
                <el-table-column prop="status" label="Status">
                    <template #default="scope">
                        <el-tag type="warning" v-if="scope.row.status == 1">Pending</el-tag>
                        <el-tag type="success" v-else-if="scope.row.status == 2">Approved</el-tag>
                        <el-tag type="danger" v-else-if="scope.row.status == 3">Rejected</el-tag>
                    </template>
                </el-table-column>
                <el-table-column fixed="right" label="Actions" width="150">
                    <template #default="scope">

                        <el-tooltip class="box-item" effect="dark" content="Details" placement="bottom">
                            <el-button size="small" :icon="Tickets" circle @click="showDetailDialog(scope.row)">
                            </el-button>
                        </el-tooltip>

                        <el-tooltip v-if="scope.row.status == 1" class="box-item" effect="dark" content="Review"
                            placement="bottom">
        ...
        <el-dialog v-model="detailDialogVisible" title="Comment Details" width="700">
            <el-form :model="commentDetail" label-width="auto">
                ...
                <el-form-item label="Status">
                    <el-tag type="warning" v-if="commentDetail.status == 1">Pending</el-tag>
                    <el-tag type="success" v-else-if="commentDetail.status == 2">Approved</el-tag>
                    <el-tag type="danger" v-else-if="commentDetail.status == 3">Rejected</el-tag>
                </el-form-item>
        ...
        <FormDialog ref="editDialogRef" title="Review Comment" destroyOnClose @submit="onSubmit">
            <el-form ref="formRef" :rules="rules" :model="form" label-width="auto">
                <el-form-item label="Status" prop="status">
                    <el-radio-group v-model="form.status">
                        <el-radio label="2">Approve</el-radio>
                        <el-radio label="3">Reject</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="Reason" prop="reason" v-if="form.status == 3">
                    <el-input type="textarea" placeholder="Please enter the reason for rejection" v-model="form.reason" :rows="6" />
                </el-form-item>
            </el-form>
        </FormDialog>
    </div>
